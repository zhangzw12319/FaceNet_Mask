...
# -*- coding: utf-8 -*-
"""
Created on Wed May 26 17:43:12 2021
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_root", help="onnx model root, default is './'", default="/media/zhangzhizhong/wjm/code/insightface_baseline_torch/ms1mv3_arcface_r50")
    args = parser.parse_args()
    test = ArcFaceORT(args.model_root)
    ...
    item = read_record.read_idx(25000)
    header, img_bytes = mx.recordio.unpack(item)
    img_numpy = mx.image.imdecode(img_bytes).asnumpy()
    print(test.check(test_img=img_numpy))
    print(test.forward(img_numpy).shape)
    print(test.benchmark(img_numpy)[0].shape)
    print(test.benchmark(img_numpy)[1])